...
export class GenericTable {
    ...
    private createSingleLambda(lambdaName: string): NodejsFunction {
        ...
        return new NodejsFunction(this.stack, lambdaId, {
            entry: (join(__dirname, '..', 'services', 'a.ts')),
            handler: 'handler',
    ...
    private grantTableRights() {
        if(this.createLambda){
            this.table.grantFullAccess(this.createLambda);
        }
        if(this.readLambda){
            this.table.grantFullAccess(this.readLambda);
        }
        if(this.updateLambda){
            this.table.grantFullAccess(this.updateLambda);
        }
        if(this.deleteLambda){
            this.table.grantFullAccess(this.deleteLambda);
        }
