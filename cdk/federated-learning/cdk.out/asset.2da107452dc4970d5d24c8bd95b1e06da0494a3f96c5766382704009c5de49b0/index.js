...
var Analysis = class {
  constructor(name, description, createdAt = new Date()) {
    this.name = name;
  ...
  key() {
    return {
      "PK": { "S": `ANALYSIS#${this.analysisId}` },
      "SK": { "S": `ANALYSIS#${this.analysisId}` }
    };
  }
  toItem() {
    return {
...
// src/repositories/analysis-repo.ts
var import_aws_sdk = require("aws-sdk");
var _AnalysisRepo = class {
  constructor() {
    this.dynamodbClient = new import_aws_sdk.DynamoDB.DocumentClient({ apiVersion: "2012-08-10" });
  }
    ...
    const transactionItems = [
      {
        Put: {
          ...
          TableName: _AnalysisRepo.TABLE_NAME,
          ConditionExpression: "attribute_not_exists(PK)"
        }
      }
    ];
    this.executeTransactionWrite({
      TransactItems: transactionItems
    });
  }
  executeTransactionWrite(params) {
    const transactionRequest = this.dynamodbClient.transactWrite({
...
var AnalysisRepo = _AnalysisRepo;
AnalysisRepo.TABLE_NAME = "federated-learning-table";
...
async function handler(event, context) {
  ...
  if (event.httpMethod === "GET") {
    ...
    if (event.body) {
      ...
      new AnalysisRepo().createAnalysis(analysis);
      return {
        statusCode: 200,
        body: JSON.stringify({
          data: {
