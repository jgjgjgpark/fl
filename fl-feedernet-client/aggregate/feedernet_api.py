...
class API:
    ...
    DOWNLOAD_FILE_PATH = "../download/result_{}_{}_{}_{}_{}.zip"
    ...
    def API_LIST_CDM(self, project_id, analysis_id):
        ...
        print("API_LIST_CDM URL : ", url)
        res = requests.get(url, cookies = self.LOGIN_COOKIES)
        print("API_LIST_CDM : RESULT : ", res.json())
        return res.json()
    ...
    def API_LIST_CONTAINER_IMAGES(self):
        ...
        print("API_LIST_CONTAINER_IMAGES URL : ", url)
        res = requests.get(url, cookies = self.LOGIN_COOKIES)
        print("API_LIST_CONTAINER_IMAGES : RESULT : ", res.json())
        return res.json()
    ...
    :return: status_code
    """
    def API_UPLOAD(self, project_id, analysis_id, file_path):

        print("==============================")
        url = self.URL_UPLOAD.format(project_id, analysis_id, "docker.evidnet.co.kr/base-images/evidnet/tensorflow:latest")
  ...
  'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOjIzLCJscG0iOjE1OTg0ODU0MzQwNDMsInRzIjoxNjI3NDU4MzcxNDQxLCJpYXQiOjE2Mjc0NTgzNzB9.zi0uqmThijhIo9GNZRNOBpb5lX-WKJngLAuPKUkPFJo'
    ...
    """
    def API_GET_RESULT_FILE(self, project_id, analysis_id, cdm_id, execution_id, file_id, current_round):

        print("==============================")
        #result_file_path = self.OUTPUT_FILE_PATH.format(project_id, analysis_id, execution_id, current_round)
        result_file_path = self.DOWNLOAD_FILE_PATH.format(project_id, analysis_id, cdm_id, execution_id, current_round)
        ...
        """
        test_file_path = "../download/result.zip"
        test_result_file_path = self.DOWNLOAD_FILE_PATH.format(project_id, analysis_id, cdm_id, execution_id, current_round)
        shutil.copy(test_file_path, test_result_file_path)

        return 200, test_result_file_path
        """
